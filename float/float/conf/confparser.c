...
int32_t confparser_serialize_float_config(uint8_t *buffer, const float_config *conf) {
	...
	buffer_append_float16(buffer, conf->buzz_current, 100, &ind);
	buffer[ind++] = (uint8_t)conf->buzz_period_1;
	buffer[ind++] = (uint8_t)conf->buzz_period_2;
	buffer_append_float16(buffer, conf->buzz_time, 1, &ind);
	buffer_append_float16(buffer, conf->buzz_motor_current, 1, &ind);
	buffer_append_float16(buffer, conf->buzz_battery_current, 1, &ind);
	buffer_append_float16(buffer, conf->buzz_duty_cycle, 1, &ind);
...
bool confparser_deserialize_float_config(const uint8_t *buffer, float_config *conf) {
	...
	conf->buzz_current = buffer_get_float16(buffer, 100, &ind);
	conf->buzz_period_1 = buffer[ind++];
	conf->buzz_period_2 = buffer[ind++];
	conf->buzz_time = buffer_get_float16(buffer, 1, &ind);
	conf->buzz_motor_current = buffer_get_float16(buffer, 1, &ind);
	conf->buzz_battery_current = buffer_get_float16(buffer, 1, &ind);
	conf->buzz_duty_cycle = buffer_get_float16(buffer, 1, &ind);
...
void confparser_set_defaults_float_config(float_config *conf) {
	...
	conf->buzz_current = APPCONF_FLOAT_BUZZ_CURRENT;
	conf->buzz_period_1 = APPCONF_FLOAT_BUZZ_PERIOD_1;
	conf->buzz_period_2 = APPCONF_FLOAT_BUZZ_PERIOD_2;
	conf->buzz_time = APPCONF_FLOAT_BUZZ_PERIOD_1;
	conf->buzz_motor_current = APPCONF_FLOAT_BUZZ_MOTOR_CURRENT;
	conf->buzz_battery_current = APPCONF_FLOAT_BUZZ_BATTERY_CURRENT;
	conf->buzz_duty_cycle = APPCONF_FLOAT_BUZZ_DUTY_CYCLE;
