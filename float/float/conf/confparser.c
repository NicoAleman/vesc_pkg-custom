...
int32_t confparser_serialize_float_config(uint8_t *buffer, const float_config *conf) {
	...
	buffer_append_float16(buffer, conf->inputtilt_brake1_scale_threshold, 10000, &ind);
	buffer_append_float16(buffer, conf->inputtilt_brake2_scale_threshold, 10000, &ind);
	buffer[ind++] = conf->inputtilt_kp1_is_scaled;
	buffer[ind++] = conf->inputtilt_kp2_is_scaled;
	buffer_append_float16(buffer, conf->remote_booster_kp_accel, 10, &ind);
	buffer_append_float16(buffer, conf->remote_booster_kp_brake, 10, &ind);
	buffer_append_float16(buffer, conf->remote_booster_ramp_factor, 10, &ind);
...
bool confparser_deserialize_float_config(const uint8_t *buffer, float_config *conf) {
	...
	conf->inputtilt_brake1_scale_threshold = buffer_get_float16(buffer, 10000, &ind);
	conf->inputtilt_brake2_scale_threshold = buffer_get_float16(buffer, 10000, &ind);
	conf->inputtilt_kp1_is_scaled = buffer[ind++];
	conf->inputtilt_kp2_is_scaled = buffer[ind++];
	conf->remote_booster_kp_accel = buffer_get_float16(buffer, 10, &ind);
	conf->remote_booster_kp_brake = buffer_get_float16(buffer, 10, &ind);
	conf->remote_booster_ramp_factor = buffer_get_float16(buffer, 10, &ind);
...
void confparser_set_defaults_float_config(float_config *conf) {
	...
	conf->inputtilt_brake1_scale_threshold = APPCONF_FLOAT_INPUTTILT_BRAKE1_SCALE_THRESHOLD;
	conf->inputtilt_brake2_scale_threshold = APPCONF_FLOAT_INPUTTILT_BRAKE2_SCALE_THRESHOLD;
	conf->inputtilt_kp1_is_scaled = APPCONF_FLOAT_INPUTTILT_KP1_IS_SCALED;
	conf->inputtilt_kp2_is_scaled = APPCONF_FLOAT_INPUTTILT_KP2_IS_SCALED;
	conf->remote_booster_kp_accel = APPCONF_FLOAT_REMOTE_BOOSTER_KP_ACCEL;
	conf->remote_booster_kp_brake = APPCONF_FLOAT_REMOTE_BOOSTER_KP_BRAKE;
	conf->remote_booster_ramp_factor = APPCONF_FLOAT_REMOTE_BOOSTER_RAMP_FACTOR;
